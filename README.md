...
Text edits use a guarded preview transaction. BarebonesPDF first generates a separate in-memory PDF, blocks structural damage, unreadable output, missing replacement text, and text outside the page boundary, then reopens the result and compares before/after renders. Ordinary font-metric or render-mask differences are shown as warnings rather than blocking the edit. A before/after preview and any standard-font substitution are shown before **Apply Validated Edit** becomes available. Until that button is chosen, the open document is unchanged.
