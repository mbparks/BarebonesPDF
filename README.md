...
Text edits use a guarded preview transaction. BarebonesPDF first generates a separate in-memory PDF, rejects replacement lines that exceed their permitted geometry, reopens the result, verifies page structure and extracted replacement text, and compares before/after page renders outside the selected region. A before/after preview and any standard-font substitution are shown before **Apply Validated Edit** becomes available. Until that button is chosen, the open document is unchanged.
